  ...
  return (
    <>
      <button
        ...
        onClick={onClose}
      />
      <section className="fixed inset-x-0 bottom-0 z-50 max-h-[80vh] rounded-t-3xl border border-cream-200 bg-cream-50 p-4 shadow-2xl animate-sheet-up dark:border-charcoal-700 dark:bg-charcoal-900">
        ...
        <ul className="space-y-2 overflow-y-auto pb-[calc(1rem+env(safe-area-inset-bottom))]">
          {recipes.map((recipe) => (
            <li key={recipe.id}>
              <button
                type="button"
